...
<body>
  ...
  <main class="board">
    ...
    if (!empty($_GET['errCode'])) {
      ...
      if ($code === '2') {
        echo '<h2 class="remind"> 帳號或密碼已有人註冊，請更換一組</h2>';
      }
    }
    ?>
    <div class="btn_group">
    ...
    <form class="board_input-area" method='POST' action="handle_register.php">
      <div class="input-area_username input-area">
        帳號：<input type="text" name='username' class="input">
      </div>
      <div class="input-area_password input-area">
        密碼：<input type="password" name='password' class="input">
      </div>
      <div class="input-area_nickname input-area">
        暱稱：<input type="text" name='nickname' class="input">
      </div>
